...
public class AbilityManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.E) || Gamepad.current.leftTrigger.wasPressedThisFrame) && Controller.abilities[0] != null) // activate first ability slot
        {
            abilitySwitcher[Controller.abilities[0].getName()].activateAbility();
        }
        
        if ((Input.GetKeyDown(KeyCode.R) || Gamepad.current.buttonNorth.wasPressedThisFrame) && Controller.abilities[1] != null) // activate second ability slot
        {
            abilitySwitcher[Controller.abilities[1].getName()].activateAbility();
        }
        
        if ((Input.GetKeyDown(KeyCode.F) || Gamepad.current.buttonEast.wasPressedThisFrame) && Controller.abilities[2] != null) // activate third ability slot
        {
